...
class Logger:
    ...
    def serve(self):
        ...
        # SUDAH BENAR: Bind ke 0.0.0.0 agar bisa menerima koneksi dari luar
...
class Gossip:
    ...
    Sends gossip to each peer's UDP.
    MODIFIED: Accepts 'my_host' to advertise correct IP instead of localhost.
    """
    def __init__(self, node_id:int, udp_port:int, peers_map:List[Tuple[str,int,int,int]], my_host:str='127.0.0.1'):
        self.id=node_id; self.udp_port=udp_port
        # MODIFIKASI: Menggunakan my_host (IP asli) bukan hardcoded 127.0.0.1
        self.table: Dict[int, Dict]= {
            self.id:{'state':STATE_ALIVE,'hb':0,'last':time.monotonic(),'addr':(my_host,None)}
        }
    ...
    def _tx(self):
        while True:
            ...
            # MODIFIKASI: Saat mengirim gossip, 'addr' default akan mengambil dari self.table[self.id] yang sudah benar (IP asli)
    ...
    def addr_of(self,nid:int)->Optional[Tuple[str,int]]:
        ...
        if nid==self.id:
            # Fallback jika mencari diri sendiri
            for h,tcp,_udp,i in self.peers_map:
                if i==self.id: return (h,tcp)
            # Return address from gossip table if available
            return self.table[self.id].get('addr')
        return None
...
class Node:
    def __init__(self, node_id:int, tcp_port:int, udp_port:int, peers_map:List[Tuple[str,int,int,int]],
                 logger_addr:Tuple[str,int], numnodes:int, use_mutex:bool, my_host:str='127.0.0.1'):
        self.id=node_id; self.tcp_port=tcp_port; self.use_mutex=use_mutex
        
        # MODIFIKASI: Gunakan my_host untuk identitas diri sendiri di peers map
        if not any(i==node_id for *_, i in peers_map):
            peers_map=[(my_host, tcp_port, udp_port, node_id)] + peers_map
            
        # MODIFIKASI: Pass my_host ke Gossip agar tabel gossip benar
        self.gossip=Gossip(node_id, udp_port, peers_map, my_host=my_host)
...
def main():
    ...
    ap.add_argument('--host', type=str, default='127.0.0.1', help='My own IP address') # MODIFIKASI: Tambah argumen host
    ...
    # MODIFIKASI: Pass args.host ke Node
    Node(args.id, args.tcp, args.udp, peers, logger_addr, args.numnodes, bool(args.use_mutex), my_host=args.host)
    # Keep process alive
    while True:
        time.sleep(3600)
